## SI 364 - Winter 2018
## HW 3
## Worked with Nicole Greenberg, John Falcon, Danielle Meyerson
...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgres://laurensigurdson@localhost:5432/lsigurdHW3"
...
def index():
    ...
    tweetObj = Tweet.query.all() #specifies that this is going to be a query that returns Tweet objects
    numTweets = len(tweetObj)
    ...
    ## Or if there is not, then create one and add it to the database     
        ...
        user = User.query.filter_by(username = username).first() # user either equals True or None
        if not user: #if there is nothing in user
    ...
    ## And redirect to the list of all tweets
        text = form.text.data
        existing_tweet = Tweet.query.filter_by(text = text, user_id = user.id).first()
        if existing_tweet:
            flash('Sorry, you already tweeted this!')
    ...
    ## Flash a message about a tweet being successfully added
        flash('Tweet has been successfully tweeted and added to the database.')
    ...
    return render_template('index.html', num_tweets = numTweets, form = form) 
...
def see_all_tweets():
    ...
    for tweet in all_tweets:
        ...
        tweet_users.append((tweet.text, user.username))
...
def get_longest_tweet():
    ...
    current_tweet = None
    for tweet in all_tweets:
        if len(tweet.text) > max_length:
            max_length = len(tweet.text)
            current_tweet = tweet
    user = User.query.filter_by(id = current_tweet.user_id).first()
    # pass back current_tweet and user 
    return render_template('longest_tweet.html', longest_tweet = current_tweet, user = user)
